...
public class JarCache {
     ...
     * @param requirePrefix The domain-specific prefix to be expected (ex. "gr.wavesoft.")
     */
    private static int importJAR(String filename, String requirePrefix) throws IOException {
        ...
        try {
            ...
            WebNGIO.systemDB.update("INSERT INTO jar_files (file,expire) VALUES (?,?)", filename, System.currentTimeMillis() + 3600000l);
            ResultSet res = WebNGIO.systemDB.query("SELECT id FROM jar_files ORDER BY id DESC LIMIT 0,1");
            if (!res.next()) {
            ...
            // Prepare statement
            PreparedStatement stmtCLS = WebNGIO.systemDB.prepareStatement("INSERT INTO jar_cache (class,jar_id,expire) VALUES (?,?,?)");
    ...
    private static String lookupJAR(String className) {
        try {
            ...
            ResultSet res = WebNGIO.systemDB.query("SELECT jar_files.file "
                    ...
                    downloadJar(jarURL, new AsyncEventListener() {
                        ...
                        public void completed(Object result) {
                            try {
                                ...
                                WebNGIO.systemDB.update("UPDATE jar_lookup SET jar_id = ? WHERE url = ?", jarID, jarURL.toString());
