...
import gr.wavesoft.webng.io.DownloadManager.DownloadListener;
...
public class JarDiscovery {
    ...
    private static URL lookupJAR(String fqn) {
        ...
        WebNGIO.systemDB.update("DELETE FROM jar_lookup WHERE expire < ?", System.currentTimeMillis());
        
        // Fetch jar
        PreparedStatement s = WebNGIO.systemDB.prepareStatement("SELECT url FROM jar_lookup WHERE ? LIKE match ORDER BY LENGTH(match) DESC LIMIT 0,1");
        try {
    ...
    private static URL lookupExports(String fqn) {
        ...
        PreparedStatement s = WebNGIO.systemDB.prepareStatement("SELECT url FROM exports_lookup WHERE ? LIKE match ORDER BY LENGTH(match) DESC LIMIT 0,1");
        try {
    ...
    private static Boolean updateExports(URL exportsFile) {
        ...
        PreparedStatement stmtJAR = WebNGIO.systemDB.prepareStatement("INSERT INTO jar_lookup (match,url,expire) VALUES (?,?,?)");
        PreparedStatement stmtEXPORTS = WebNGIO.systemDB.prepareStatement("INSERT INTO exports_lookup (match,url,expire) VALUES (?,?,?)");
    ...
    public static int locateLocalJarID(String className) {
        ...
        // Fetch jar
        PreparedStatement s = WebNGIO.systemDB.prepareStatement("SELECT jar_id FROM jar_lookup WHERE ? LIKE match ORDER BY LENGTH(match) DESC LIMIT 0,1");
        try {
